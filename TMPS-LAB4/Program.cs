    ...
    class Program
    {
        ...
        private static bool MainMenu()
        {
            ...
            switch (Console.ReadLine())
            {
                ...
                case "1":
                    Console.Clear();
                    Console.WriteLine("Director general: {1} {2} | Role: {3} | Salary : {4}", Owner.EmployeId, 
                        Owner.FirstName, Owner.LastName, Owner.Role, ownerSalary.CountSalary());

                    foreach (EmployeeService manager in Owner)
                    {
                        Console.WriteLine("\n\n\t{0}\n", manager.Role);
                        Console.WriteLine("\n\t SEF DEPARTAMENT: {1}  {2} | Role: Director al {3} | Salary : {4}", manager.EmployeId,
                            manager.FirstName, manager.LastName, manager.Role, departmentSalary.CountSalary());

                        foreach (EmployeeService employee in manager)
                        {
                            Console.WriteLine(" \t\t ID Angajat: {0} | First Name: {1} | Last Name: {2} | Role: {3} | Salary : {4}", employee.EmployeId, employee.FirstName,
                                employee.LastName, employee.Role, counterSalary.CountSalary());

                            foreach (var simpleEmployee in employee)
                            {
                                Console.WriteLine("\t\t\t ID Angajat: {0} | First Name: {1} | Last Name: {2} | Role: {3} | Salary : {4}\n", simpleEmployee.EmployeId, simpleEmployee.FirstName,
                                    simpleEmployee.LastName, simpleEmployee.Role, salary.CountSalary());
                            }
                        }
                    }
                    Console.ReadKey();
                ...
                case "2":
                    ...
                    foreach (EmployeeService manager in Owner)
                    {
                        ...
                        {
                            Console.WriteLine(" \t\t ID Angajat: {0} | First Name: {1} | Last Name: {2} | Role: {3} | Salary : {4}", employee.EmployeId, employee.FirstName,
                                employee.LastName, employee.Role, managerSalary.CountSalary());
